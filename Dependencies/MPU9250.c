 ...
 *
 * Converted from C++ to C for ECE department by Bernie Roehl, February 2017
 *
 */
...
float MPU9250_accel_data[3];
float MPU9250_temperature;
float MPU9250_gyro_data[3];
float MPU9250_mag_data[3];
int16_t mag_data_raw[3];    
uint8_t MPU9250_st_value;
...
void MPU9250_ReadRegs(uint8_t ReadAddr, uint8_t *ReadBuf, unsigned int Bytes)
{
    unsigned int i = 0;
    SPI_select();
    SPI_xfer(ReadAddr | READ_FLAG);
    for (i = 0; i < Bytes; i++)
        ReadBuf[i] = SPI_xfer(0);
    SPI_deselect();
}
...
BOOL MPU9250_init(BOOL calib_gyro, BOOL calib_acc){
    float temp[3];
    uint8_t i = 0;
    ...
    if (calib_gyro && calib_acc){
        ...
        MPU9250_calibrate(g_bias, temp);
    }
    else if (calib_acc){
        MPU9250_calibrate(temp, a_bias);
    }
    

    for (i = 0; i < MPU_InitRegNum; i++) {
        MPU9250_WriteReg(MPU_Init_Data[i][1], MPU_Init_Data[i][0]);
        DelayMilliseconds(10);  // I2C must slow down the write speed, otherwise it won't work
    }
...
unsigned int MPU9250_whoami(){
    ...
    response = MPU9250_WriteReg(MPUREG_WHOAMI|READ_FLAG, 0x00);
    return response;
}
...
void MPU9250_read_acc()
{
    ...
    float data;
    int i;
    MPU9250_ReadRegs(MPUREG_ACCEL_XOUT_H,response,6);
    for (i = 0; i < 3; i++) {
        bit_data = ((int16_t) response[i*2]<<8)|response[i*2+1];
        data = (float) bit_data;
        MPU9250_accel_data[i] = data/acc_divider - a_bias[i];
    }
...
void MPU9250_read_gyro()
{
    ...
    float data;
    int i;
    MPU9250_ReadRegs(MPUREG_GYRO_XOUT_H,response,6);
    for (i = 0; i < 3; i++) {
        bit_data = ((int16_t) response[i*2]<<8) | response[i*2+1];
        data = (float)bit_data;
        MPU9250_gyro_data[i] = data/gyro_divider - g_bias[i];
    }
}
...
void MPU9250_read_temp(){
    ...
    int16_t bit_data;
    float data;
    MPU9250_ReadRegs(MPUREG_TEMP_OUT_H,response,2);

    bit_data = ((int16_t)response[0]<<8)|response[1];
    data = (float)bit_data;
    MPU9250_temperature = (data/340)+36.53;
    SPI_deselect();
}
...
uint8_t MPU9250_AK8963_whoami(){
    uint8_t response;
    MPU9250_WriteReg(MPUREG_I2C_SLV0_ADDR,AK8963_I2C_ADDR|READ_FLAG); //Set the I2C slave address of AK8963 and set for read.
	...
	DelayOneMillisecond();
    //MPU9250_ReadRegs(MPUREG_EXT_SENS_DATA_00,response,1);
...
void MPU9250_calib_mag(){
	...
	DelayMilliseconds(50);							    // long wait between AK8963 mode changes
...
void MPU9250_read_mag(){
    ...
    MPU9250_WriteReg(MPUREG_I2C_SLV0_CTRL, 0x87);                      // Read 6 bytes from the magnetometer plus the ST2 byte
    ...
    for (i = 0; i < 3; i++) {
        ...
        MPU9250_mag_data[i] = data*Magnetometer_ASA[i];
    }
	MPU9250_st_value = response[6];
}
...
void MPU9250_read_all(){
    ...
    bit_data = ((int16_t)response[i*2]<<8) | response[i*2+1];
    data = (float)bit_data;
    MPU9250_temperature = ((data-21)/333.87)+21;
    // Get gyroscope value
    for (i=4; i < 7; i++) {
        bit_data = ((int16_t)response[i*2]<<8) | response[i*2+1];
        data = (float)bit_data;
        MPU9250_gyro_data[i-4] = data/gyro_divider - g_bias[i-4];
    }
    // Get Magnetometer value
    for (i=7; i < 10; i++) {
        mag_data_raw[i] = ((int16_t)response[i*2+1]<<8) | response[i*2];
        data = (float)mag_data_raw[i];
        MPU9250_mag_data[i-7] = data * Magnetometer_ASA[i-7];
    }
}
...
void MPU9250_calibrate(float *dest1, float *dest2){  
    ...
    DelayMilliseconds(40); // accumulate 40 samples in 40 milliseconds = 480 bytes
